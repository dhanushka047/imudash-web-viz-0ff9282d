...
export function useBLE() {
  ...
  const addOrUpdate = useCallback(
    (d: Partial<SeenDevice> & { id: string }) => {
      setDevices((prev) => {
        const old = prev[d.id];
  ...
  const startScan = useCallback(async () => {
    ...
    if (!supportsScanning()) {
      setScanError(
        "BLE scanning not supported. Use the device picker instead."
      );
      return;
    }
    try {
      ...
      const onAdv = (event: any) => {
        try {
          const id: string =
            event?.device?.id ?? event?.uuid ?? crypto.randomUUID();
          const name: string =
  ...
  const onNotify = (ev: Event) => {
    ...
    // Keep full hex preview for debugging
  ...
  const connect = useCallback(
    async (device: BluetoothDevice) => {
      ...
      try {
        ...
        if (!service) {
          setConnectMessage(
            "Not found valid UUID (service). Disconnecting…"
          );
          disconnect();
          return false;
        }
        ...
        if (!notifyChar) {
          setConnectMessage(
            "Not found valid UUID (characteristic). Disconnecting…"
          );
          disconnect();
          return false;
        }
